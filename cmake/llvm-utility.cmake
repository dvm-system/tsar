...
#      If BUILD_CLANG is set then ${CLANG_SOURCE_DIR} and ${CLANG_REPOS} must be also set.
...
#       If BUILD_CLANG is set check out a working copy ${CLANG_SOURCE_DIR}
#       from Clang repository ${CLANG_REPO} or update it if the working copy already exist.
function(sapfor_download_llvm)
    if(NOT EXISTS ${LLVM_SOURCE_DIR})
        ...
        if(BUILD_CLANG AND NOT LLVM_ERROR)
            message(STATUS "${LLVM_STATUS} - done")
            set(LLVM_STATUS "Checking out Clang working copy")
            message(STATUS ${LLVM_STATUS})
            execute_process(COMMAND ${Subversion_SVN_EXECUTABLE} co ${CLANG_REPO} 
                                    ${CLANG_SOURCE_DIR} RESULT_VARIABLE LLVM_ERROR)
        endif()
    else()
        ...
        if(BUILD_CLANG AND NOT LLVM_ERROR)
            ...
            message(STATUS ${LLVM_STATUS})
            execute_process(COMMAND ${Subversion_SVN_EXECUTABLE} up ${CLANG_SOURCE_DIR}
                                    RESULT_VARIABLE LLVM_ERROR)
        endif()
    endif()    
    if(LLVM_ERROR)
