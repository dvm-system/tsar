...
std::pair<const SCEV *, const SCEV *> Subscript::getСoefficients(ScalarEvolution &SE) {
  if (!mIsCoefficientsCounted) {
    mСoefficients = findCoefficientsInSCEV(mExpr, SE);
    mIsCoefficientsCounted = true;
  }
  return mСoefficients;
}

bool Subscript::isConst(ScalarEvolution &SE) {
  getСoefficients(SE);
  return isa<SCEVConstant>(mСoefficients.first)
    && isa<SCEVConstant>(mСoefficients.second);
}
  ...
  for (auto &CurrentArray : mAnalyzedArrays) {
    for (auto &CurrentAccess : CurrentArray.mAccesses) {
      ...
      for (auto &CurrentSubscript : CurrentAccess.mSubscripts) {
        CurrentCoefficients.push_back(CurrentSubscript.getСoefficients(SE));
      }
    ...
    for (auto &ArrayEntry : mAnalyzedArrays) {
      ...
      for (auto &AccessEntry : ArrayEntry.mAccesses) {
        for (auto &S : AccessEntry.mSubscripts) {
          ...
          auto Idxs = S.getСoefficients(SE);
          if (Idxs.first) {
            dbgs() << "\ta: ";
            Idxs.first->dump();
          } else {
